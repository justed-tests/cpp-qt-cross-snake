...
function createSnake () {
  ...
  for (var i = 0; i < initialSnakeSize; i++) {
    ...
    initialPosition.x +i, initialPosition.y))
  }
}
...
function createWalls () {
  ...
  for (var y = 0; y < board.rowCount - 1; y++) {
...
function spawnApple() {
  ...
  while (!isFound) {
    position = Qt.vector2d(
      Math.floor(Math.random() * board.columnCount),
...
function setPosition (item, column, row) {
  ...
  item.gridPosition(item, column, row)
...
function handleKeyEvent (event) {
  switch (event.key) {
    case Qt.Key_R:
      ...
      break
    case Qt.Key_I:
      if (direction != Qt.vector2d(0, -1)) direction = Qt.vector2d(0, 1)
      break
    case Qt.Key_K:
      if (direction != Qt.vector2d(0, 1)) direction = Qt.vector2d(0, -1)
      break
    case Qt.Key_J:
      if (direction != Qt.vector2d(1, 0)) direction = Qt.vector2d(-1, 0)
      break
    case Qt.Key_L:
      if (direction != Qt.vector2d(-1, 0)) direction = Qt.vector2d(1, 0)
      break
...
function upate () {
  if (gameRoot.state == "GAMEOVER") return

  var headPosition = snake[0].gridPosition
  var newPosition = Qt.vector2d(
  ...
  switch (itemOnNewPosition.type) {
    ...
    case Factory.APPLE_TYPE:
      ...
      snake.unshift(factory.createGameEntity(
        Factory.SNAKE_TYPE, newPosition.x,
        newPosition.y
      ))
